...
class Aplicacion():
    def __init__(self):
        ...
        for i, color in enumerate(self.colores):
            ...
            # Agrego el boton que se creo en una lista de botones que son canvas en realidad
        ...
        self.__ventana.bind('<Return>', lambda event: self.ventana_jugador())  # Evento para tecla Enter
        ...
        # Vincular teclas a la función de procesamiento de teclas
        self.teclas_colores = {
            'v': 0,  # verde
            'r': 1,  # rojo
            'm': 2,  # amarillo
        ...
        self.__ventana.bind('<Key>', self.procesar_tecla)
    ...
    def generar_color(self):
        if self.secuencia == []:  # primer secuencia inicia con verde si la lista esta vacia
    ...
    def mostrar_secuencia(self):
        time.sleep(1)  # demora el programa 1 segundo
        for color in self.secuencia:  # itero sobre cada color guardado en la secuencia
            index_color = self.colores.index(color)  # encuentro el indice actual del self.secuencia
    ...
    def jugar(self):
        ...
        self.__ventana.bind("<Button-1>", self.procesar_clic)  # cada vez que haga click llama al metodo self.procesar_click

    def procesar_clic(self, event):
        colores_click = ['#7fbf80', '#df5f5f', '#ffff80', '#7f7fbf']  # colores que sirven para dar el efecto de boton clickeado
        boton_presionado = event.widget  # guardo la referencia del boton clickeado
        color_presionado = boton_presionado.cget("background")  # guardo el color que fue presionado por el jugador
    ...
    def procesar_tecla(self, event):
        tecla = event.char
        if tecla in self.teclas_colores:
            index = self.teclas_colores[tecla]
            self.presionar_boton(index)

    def presionar_boton(self, index):
        colores_click = ['#7fbf80', '#df5f5f', '#ffff80', '#7f7fbf']
        boton_presionado = self.botones[index]
        color_presionado = self.colores[index]

        self.respuesta_jugador.append(color_presionado)
        boton_presionado.config(bg=colores_click[index])
    ...
    def actualizar_puntaje(self, puntos):
        ...
        self.puntaje.config(text=str(self.__puntaje))  # actualizo en el label el nuevo puntaje obtenido
    ...
    def ventana_jugador(self):
        ...
        self.__ventanaJug.bind('<Control_L>', lambda event: self.actualizarNombre())  # Evento para tecla control
